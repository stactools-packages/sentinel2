...
SCHEMA_URI: Final[
    str] = "https://stac-extensions.github.io/mgrs/v1.0.0/schema.json"
PREFIX: Final[str] = "mgrs:"

# Field names
LATITUDE_BAND_PROP: Final[str] = PREFIX + "latitude_band"  # required
GRID_SQUARE_PROP: Final[str] = PREFIX + "grid_square"  # required
UTM_ZONE_PROP: Final[str] = PREFIX + "utm_zone"

LATITUDE_BANDS: Final[FrozenSet[str]] = frozenset({
    'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S',
    'T', 'U', 'V', 'W', 'X'
})

UTM_ZONES: Final[FrozenSet[int]] = frozenset({
...
GRID_SQUARE_REGEX: Final[str] = \
    r"[ABCDEFGHJKLMNPQRSTUVWXYZ][ABCDEFGHJKLMNPQRSTUV](\d{2}|\d{4}|\d{6}|\d{8}|\d{10})?"
GRID_SQUARE_PATTERN: Final[Pattern[str]] = re.compile(GRID_SQUARE_REGEX)
...
def validated_latitude_band(v: str) -> str:
    ...
    if v not in LATITUDE_BANDS:
        raise ValueError(
            f"Invalid MGRS latitude band: {v} is not in {LATITUDE_BANDS}")
    return v
...
def validated_utm_zone(v: Optional[int]) -> Optional[int]:
    if v is not None and not isinstance(v, int):
        raise ValueError("Invalid MGRS utm zone: must be None or str")
...
class MgrsExtension(
    ...
    def latitude_band(self, v: str) -> None:
        self._set_property(LATITUDE_BAND_PROP,
                           validated_latitude_band(v),
                           pop_if_none=False)
    ...
    def grid_square(self, v: str) -> None:
        self._set_property(GRID_SQUARE_PROP,
                           validated_grid_square(v),
                           pop_if_none=False)
    ...
    def utm_zone(self, v: Optional[int]) -> None:
        self._set_property(UTM_ZONE_PROP,
                           validated_utm_zone(v),
                           pop_if_none=False)
    ...
    def ext(cls,
            obj: pystac.Item,
            add_if_missing: bool = False) -> "MgrsExtension":
